...
dataset, _, model = ssumo.get.data_and_model(
    ...
    data_keys=["x6d", "root", "offsets", "fluorescence"],
    shuffle=False,
    verbose=0,
)
...
for disentangle_key in model.disentangle_keys:
    for sample_i in sample_idx:
        data = dataset[sample_i]
        data = {k: v.cuda()[None,...].repeat((n_shifts+1,) + tuple(np.ones(len(v.shape), dtype=int))) for k, v in data.items()}
        z_traverse = latents[sample_i:sample_i+1, :].repeat(n_shifts+1, 1).cuda()
        data["fluorescence"][1:,:] = shift.cuda()

        data_o = model.decode(z_traverse, data)
        pose = ssumo.data.dataset.fwd_kin_cont6d_torch(data_o["x6d"].reshape((-1,) + data_o["x6d"].shape[2:]), model.kinematic_tree, data["offsets"].reshape((-1,)+data["offsets"].shape[2:]), root_pos=data_o["root"].reshape((-1, 3)), do_root_R=True, ).detach().cpu().numpy()

        subtitles = ["{:2f}".format(val) for val in data["fluorescence"].detach().cpu().numpy().squeeze()]

        vis.pose.grid3D( pose, connectivity, frames=np.arange(n_shifts+1) * model.window, centered=False, subtitles=subtitles, title=dataset_label + " Data - {} Traversal".format(disentangle_key), fps=20, N_FRAMES=model.window, VID_NAME=dataset_label + "grid{}_mod.mp4".format(sample_i), SAVE_ROOT=vis_decode_path, )
