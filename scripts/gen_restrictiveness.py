...
import torch
from scrubvae.data.dataset import get_angle2D, get_frame_yaw, fwd_kin_cont6d_torch, get_speed_parts
import tqdm
...
for an_key in analysis_keys:
    ...
    if Path(pickle_path).is_file():
        ...
        epochs_to_test = [
            e for e in scrubvae.get.all_saved_epochs(path) if (e not in metrics["epochs"]) and (e>100)
        ]
        ...
        if disentanglement_key == "heading":
            metrics["JPE"] = []
        metrics["epochs"] = [e for e in scrubvae.get.all_saved_epochs(path) if (e>100)]
