...
# print(var_key)
models = read.config(CODE_PATH + "configs/exp_finals_joint_train.yaml")["heading"]
models = {
    m[0]: [m[1], m[2]] for m in models if ((m[0] != "VAE") and (m[0] != "beta-VAE"))
}
...
for i, model_key in enumerate(models.keys()):
    ...
    # if var_key == "heading":
    z = torch.cat(
        [z, avg_speed_3d_rand, heading2D_rand],
        dim=-1,
    )
    # elif var_key == "avg_speed_3d":
    ...
    gs_heading = subf[0].add_gridspec(1, 4)
    gs_speed = subf[1].add_gridspec(1,4)
    for var_ind, var_key in enumerate(["heading", "avg_speed_3d"]):
        ...
        if var_key == "heading":
            ...
            for joint_ind, part in enumerate(parts):
                ...
                dxyz[:, joint_ind] = torch.sqrt(relative_dxyz).mean(axis=(1, 2))
...
plt.savefig("./results/genres_final_joint.png", dpi=400)
plt.close()