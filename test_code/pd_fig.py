...
for var_ind, var_key in enumerate(["pd_speed", "pd_ids"]):
    ...
    for m_key in m_dict:
        ...
        if var_key == "pd_ids":
            ...
            for i, test_i in discrete_classes[~np.isin(discrete_classes,train_i)]:
                print(test_i)
                z_train = z[np.isin(ids[ids != not_id], discrete_classes[test_i])][
                    ::window
                ]
                # print(discrete_classes[test_i], print(discrete_classes[train_i]))
                y_train = pd_label[
                    np.isin(ids[ids != not_id], discrete_classes[test_i])
                ][::window].ravel()
                z_test = z[np.isin(ids[ids != not_id], discrete_classes[train_i])][
                    ::window
                ]
                ...
                y_test[
                    ...
                    pd_label[np.isin(ids[ids != not_id], discrete_classes[train_i])][
                        ::window
                    ],
                ] = 1
