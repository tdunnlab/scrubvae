...
def trace_grid(
    ...
    for i in full_pose_inds:
        ...
        for index_from, index_to in connectivity.links:
            ...
            linewidth = 3.5 - np.linspace(0, 3.1, 10)
            if i == 0:
                lw_color = np.sqrt(np.linspace(0, 0.2, 10))
                linewidth = 4.5 - np.linspace(0, 3.1, 10)
            for co, l in zip(lw_color, linewidth):
                ax.plot(
                    xs,
                    ys,
    ...
    ax.set_aspect("equal")
    # ax.axis("off")
    ax.set_xlabel("Heading Direction Traversal")
    ax.set_ylabel("Normalized Speed Traversal")
    ax.set_xticks([140 * i for i in range(5)], ["-60", "-30", "Original", "+30", "+60"])
    ax.set_yticks(
        [140 * i for i in range(5)], ["-4.5", "-3", "-1.5", "Original", "+1.5"]
    )
    ax.set_ylim(-80, 140 * 4 + 100)
    ax.set_xlim(-80, 140 * 4 + 100)
    plt.savefig("{}/vis{}_{}".format(SAVE_ROOT, vis_plane, FIG_NAME), dpi=dpi)
    plt.close()
