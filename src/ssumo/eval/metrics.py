...
def epoch_regression(
    ...
    if Path(pickle_path).is_file() and save_load:
        ...
        epochs_to_test = [
            e for e in get.all_saved_epochs(path) if (e not in metrics["epochs"])# and (e>100)
        ]
        ...
        for key in disentangle_keys:
            ...
            if method == "log_class":
                ...
                if method == "linear_cv":
                    ...
                    acc = log_class_rand_cv(StandardScaler().fit_transform(z), y_true, model.window, 5)
                    metrics[key]["Accuracy"] += [acc]

                elif method == "qda_rand_cv":
                    acc = qda_rand_cv(StandardScaler().fit_transform(z), y_true, model.window, 5)
                    print(metrics[key])
                    metrics[key]["Accuracy"] += [acc]
...
def qda_rand_cv(z, y_true, window=51, folds=5):
    ...
    for shift_i in range(2):
        ...
        kf = KFold(n_splits = folds, shuffle=True)
        for i, (train_i, test_i) in enumerate(kf.split(downsampled_z)):
            clf = QuadraticDiscriminantAnalysis().fit(downsampled_z[train_i], downsampled_y[train_i].ravel())
            y_pred = clf.predict(downsampled_z[test_i])
            acc = (downsampled_y[test_i].ravel() == y_pred).sum()/len(test_i)
            accuracy += [acc]
    print("Train Length: {}, Test Length: {}".format(len(train_i), len(test_i)))
...
def mlp_rand_cv(z, y_true, window=51, folds=5):
    ...
    for shift_i in range(2):
        ...
        kf = KFold(n_splits = folds, shuffle=True)
        for i, (train_i, test_i) in enumerate(kf.split(downsampled_z)):
            model = train_MLP(torch.tensor(downsampled_z[train_i]), downsampled_y[train_i], 200)[0]
            y_pred = model(torch.tensor(downsampled_z[test_i]).cuda()).cpu().detach().numpy()
            r2 += [r2_score(downsampled_y[test_i], y_pred)]
    print("Train Length: {}, Test Length: {}".format(len(train_i), len(test_i)))
    return r2
...
def log_class_cv(z, y_true, ids, folds=5):
    ...
    for i in range(folds):
        ...
        accuracy = (y_true[idx_test].ravel() == clf.predict(z[idx_test])).sum() / len(idx_test)
