...
arena_size = (
    None if config["model"]["arena_size"] is None else dataset.arena_size.cuda()
)
...
beta_schedule = train.trainer.get_beta_schedule(
    config["loss"]["prior"],
    config["num_epochs"] - config["load_epoch"],
...
for epoch in tqdm.trange(
    config["train"]["load_epoch"] + 1, config["train"]["num_epochs"] + 1
):
    config["loss"]["prior"] = beta_schedule[epoch - config["train"]["load_epoch"] - 1]
    print("Beta schedule: {}".format(config["loss"]["prior"]))
    for batch_idx, data in enumerate(loader):
        optimizer.zero_grad()
        data = {k: v.to(device) for k, v in data.items()}
        data_o = {"kinematic_tree": dataset.kinematic_tree}
        ...
        if config["data"]["arena_size"] is not None:
            ...
            data_o["root"] = inv_normalize_root(x_o[..., -3:], arena_size)
            data["root"] = inv_normalize_root(data["root"], arena_size)
